...
int g_in_width = WIDTH;
int g_in_height = HEIGHT;
//int g_out_width = 176;
//int g_out_height = 144;
int g_out_width = WIDTH;
int g_out_height = HEIGHT;
int g_capture_mode = MODE;
int g_cap_fmt = FORMAT;
int g_top = 0;
int g_left = 0;
int g_input = 1;
int g_capture_count = 6;
int g_rotate = 0;
...
int start_capturing(int fd_v4l)
{
		...
		buf.index = i;
				printf("VIDIOC_QUERYBUF  buffer: %d\n", i);
...
int v4l_capture_test(int fd_v4l, const char * file)
{
	...
	std_idx = V4L2_STD_PAL;
//	if(ioctl(fd_v4l, VIDIOC_S_STD, &std_idx) < 0) {
//		printf("set std format error");
//		return -1;
//	}
		...
		printf("to do VIDIOC_DQBUF\n");
        while (count-- > 0) {
				char filename[64];
				FILE *fp;
				sprintf(filename, "yuv/%03d.yuv", g_capture_count - count);
				...
				printf("Output %d frame to %s\n", g_capture_count - count, filename);
#if OUTPUT_PERFRAME
				fflush(stdin);
				fp = fopen(filename, "w+");
     	        fwrite(buffers[buf.index].start, fmt.fmt.pix.sizeimage, 1, fp);
				fclose(fp);
#endif			
