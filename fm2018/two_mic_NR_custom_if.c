...
void Two_Mic_NR_Download(unsigned short mode)
{
   ...
   switch (mode){
	...
	case 4:
        TWOMICNR_I2C_START_TRANSMISSION;
        for (i_NR_Download_para = 0; i_NR_Download_para < TWOMICNR_TEST_MAIN_PARA_LENGTH; i_NR_Download_para++)
        {
        	for (j = 0; j < TWOMICNR_I2C_DELAY; j++);
        	Two_Mic_NR_send_byte((kal_uint8)(parameter_data_TEST_MAIN_MIC[i_NR_Download_para]));
        }
        for (j = 0; j < TWOMICNR_I2C_DELAY; j++);
        TWOMICNR_I2C_STOP_TRANSMISSION;
   		break;
	case 5:
			TWOMICNR_I2C_START_TRANSMISSION;
			for (i_NR_Download_para = 0; i_NR_Download_para < TWOMICNR_TEST_SUB_PARA_LENGTH; i_NR_Download_para++)
			{
				for (j = 0; j < TWOMICNR_I2C_DELAY; j++);
				Two_Mic_NR_send_byte((kal_uint8)(parameter_data_TEST_SUB_MIC[i_NR_Download_para]));
			}
			for (j = 0; j < TWOMICNR_I2C_DELAY; j++);
			TWOMICNR_I2C_STOP_TRANSMISSION;
			break;
...
void Two_Mic_NR_chip_Init(void)
{
  ...
  /*Open main clock */
  #if defined(__BAR_S55__)
 // pmu_set_vbt_sel(VBT_SEL_3_0);
	//pmu_set_vbt_en(KAL_TRUE);   

    GPIO_ModeSetup(TWOMICNR_FM_13MHZ_GPIO_PIN, 2); /* CLKM9 mode */
    GPIO_SetClkOut(9, 4);
   //(*(volatile unsigned short *)(0x80023900)) = 0x4;	/* 13MHz */ 
  #elif defined(__SLIDE_S52__)
    GPIO_ModeSetup(TWOMICNR_FM_13MHZ_GPIO_PIN, 2); /* CLKM8 mode */
    GPIO_SetClkOut(8, 4);
  #else
 // (*(volatile unsigned short *)(0x80023200)) = 0x3;	/* 13MHz */
 // GPIO_ModeSetup(TWOMICNR_FM_13MHZ_GPIO_PIN, 2); /* CLKM2 mode */
 	GPIO_ModeSetup(TWOMICNR_FM_13MHZ_GPIO_PIN,2); /* 32.768kHz reference frequency */
	switch (TWOMICNR_FM_13MHZ_GPIO_PIN)
		  {
			  case	(0|0x80):
				  GPIO_SetClkOut(0, 4);
				  break;
...
void Two_Mic_NR_chip_TEST_MAIN_MIC_mode(void)
{
  FM_chip_Reset();
  
  /*Download handfree parameter */
  Two_Mic_NR_Download(4);
    //gary test begin
   // SET_TWOMICNR_I2C_DATA_INPUT;
   // SET_TWOMICNR_I2C_CLK_INPUT;
	//end
...
void Two_Mic_NR_chip_TEST_SUB_MIC_mode(void)
{
  FM_chip_Reset();
  
  /*Download handfree parameter */
  Two_Mic_NR_Download(5);
    //gary test begin
   // SET_TWOMICNR_I2C_DATA_INPUT;
   // SET_TWOMICNR_I2C_CLK_INPUT;
	//end
...
void Two_Mic_NR_chip_TEST_MIC_OFF_mode(void)
{
	Two_Mic_NR_chip_Sleep();
