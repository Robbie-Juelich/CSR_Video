 ...
 * 2014-07-17: Alex Hwang @ Regulus Technologies Co., Ltd.
 *  					 ==> Add essential code to control VIN5 ~ VIN8, and, AIN5 ~ AIN8 + AIN52		
 */
...
//#include "tw2968_i2c.h"
//#include "gpio_i2c.h"
//#include "gpio_i2c1.h"

#include "table_tw2968.h"
...
int init_i2c(int id)
{
	...
	if (adap == NULL) {
		printk (KERN_ERR "Could not get i2c adapter 0x%d\n", id);
		return -1;
	}
	adap->timeout = msecs_to_jiffies(2 * 10);
	adap->retries = 2;
	return 0;
}

uint8_t i2c_read(int slave, uint8_t reg)
{
	...
	if (ret != 2) 
		printk(KERN_ERR "Error read reg[0x%2x], ret : %d\n", reg, ret);

	return val;
}

void i2c_write(int slave, uint8_t reg, uint8_t data)
{
	int ret;
	char buf[2];
	...
	if (ret != 1) 
		printk(KERN_ERR "Error write reg[0x%2x: 0x%2x], ret : %d\n", 
				reg, data, ret);
...
#else
 //   return gpio_i2c_read(chip_addr,addr);
    return i2c_read(chip_addr,addr);
#endif
}
...
static int tw2968_set_work_mode_1d1(unsigned chip_addr, unsigned char ch)
{
	...
	unsigned addr = chip_addr;
#if 1
    tw2968_byte_write(addr, 0xe8, 0x01);
    tw2968_byte_write(addr, 0xe9, 0x01);
    tw2968_byte_write(addr, 0xea, 0x23);
    tw2968_byte_write(addr, 0xeb, 0x23);
    tw2968_byte_write(addr, 0xec, 0x45);
    tw2968_byte_write(addr, 0xed, 0x46);
    tw2968_byte_write(addr, 0xee, 0x67);
    tw2968_byte_write(addr, 0xef, 0x67);
#else
    tw2968_byte_write(addr, 0xe8, 0x44);
    tw2968_byte_write(addr, 0xe9, 0x44);
    tw2968_byte_write(addr, 0xea, 0x55);
    tw2968_byte_write(addr, 0xeb, 0x55);
    tw2968_byte_write(addr, 0xec, 0x66);
    tw2968_byte_write(addr, 0xed, 0x66);
    tw2968_byte_write(addr, 0xee, 0x77);
    tw2968_byte_write(addr, 0xef, 0x77);
#endif
...
#if 0
    tw2968_byte_write(chip_addr, 0xe8, 0x54);
    tw2968_byte_write(chip_addr, 0xe9, 0x76);
    tw2968_byte_write(chip_addr, 0xea, 0x10);
    tw2968_byte_write(chip_addr, 0xeb, 0x32);

    tw2968_byte_write(chip_addr, 0x63, 0x54);
	// 32
    tw2968_byte_write(chip_addr, 0x64, 0x76);
	// 76
    tw2968_byte_write(chip_addr, 0x65, 0x10);
	// ba
    tw2968_byte_write(chip_addr, 0x66, 0x32);
	// fe
#else
    tw2968_byte_write(chip_addr, 0xe8, 0x10);
    tw2968_byte_write(chip_addr, 0xe9, 0x32);
    tw2968_byte_write(chip_addr, 0xea, 0x10);
    tw2968_byte_write(chip_addr, 0xeb, 0x32);


    tw2968_byte_write(chip_addr, 0xec, 0x54);
    tw2968_byte_write(chip_addr, 0xed, 0x76);
    tw2968_byte_write(chip_addr, 0xee, 0x54);
    tw2968_byte_write(chip_addr, 0xef, 0x76);



    tw2968_byte_write(chip_addr, 0x63, 0x10);
	// 32
    tw2968_byte_write(chip_addr, 0x64, 0x32);
	// 76
    tw2968_byte_write(chip_addr, 0x65, 0x54);
	// ba
    tw2968_byte_write(chip_addr, 0x66, 0x76);
	// fe
#endif
...
#if 1
    tw2968_byte_write(chip_addr, 0xe8, 0x10);
    tw2968_byte_write(chip_addr, 0xe9, 0x10);
    tw2968_byte_write(chip_addr, 0xea, 0x32);
    tw2968_byte_write(chip_addr, 0xeb, 0x32);
    tw2968_byte_write(chip_addr, 0xec, 0x54);
    tw2968_byte_write(chip_addr, 0xed, 0x54);
    tw2968_byte_write(chip_addr, 0xee, 0x76);
    tw2968_byte_write(chip_addr, 0xef, 0x76);
#endif
#if 1
    tw2968_byte_write(chip_addr, 0x63, 0x10);
	// 32
    tw2968_byte_write(chip_addr, 0x64, 0x54);
	// 76
    tw2968_byte_write(chip_addr, 0x65, 0x98);
	// ba
    tw2968_byte_write(chip_addr, 0x66, 0xdc);
	// fe
#endif
	/* Output Enable Control and Clock Output Control */
	/* 72MHz clock output*/
...
static int tw2968_set_video_scaler(unsigned char chip_addr, unsigned long long scaler_value)
{
	...
	//-----------------------------------------
	tw2968_byte_write(chip_addr,0x40,0x01);
	...
	tw2968_byte_write(chip_addr,0x40,0x00);
	//-----------------------------------------
...
{   
	...
	if (TW2968_PAL == mode) //For WD1 only
	{
		...
		for (i=0; i<4; i++)
		{
			...
			tw2968_byte_write(addr, 0x07 + 0x10*i, 0x12);/* Cropping Register,			High */	/* D1 PAL:0x12 */
			//tw2968_byte_write(addr, 0x08 + 0x10*i, 0x1a);/* Vertical Delay Register, 		Low  */   /* D1 PAL:0x12 */
			tw2968_byte_write(addr, 0x08 + 0x10*i, 0x18);/* Vertical Delay Register, 		Low  */   /* D1 PAL:0x12 */
			//tw2968_byte_write(addr, 0x08 + 0x10*i, 0x15);/* Vertical Delay Register, 		Low  */   /* D1 PAL:0x12 */
			tw2968_byte_write(addr, 0x09 + 0x10*i, 0x20);/* Vertical Active Register, 	Low  */   /* D1 PAL:0x20 */
			tw2968_byte_write(addr, 0x0a + 0x10*i, 0x0a);/* Horizontal Delay Register, 	Low  */	/* D1 PAL:0x01 */
			tw2968_byte_write(addr, 0x0b + 0x10*i, 0xcc);/* Horizontal Active Register,	Low  */	/* D1 PAL:0xD0 */ /* D1 PAL:0xC0,For 704 */                       
			//tw2968_byte_write(addr, 0x0b + 0x10*i, 0xce);/* Horizontal Active Register,	Low  */	/* D1 PAL:0xD0 */ /* D1 PAL:0xC0,For 704 */                       
			// I changed it
//			tw2968_byte_write(addr, 0x0b + 0x10*i, 0xc5);/* Horizontal Active Register,	Low  */	/* D1 PAL:0xD0 */ /* D1 PAL:0xC0,For 704 */                       
		}
	...
	else if (TW2968_NTSC == mode)
	{
		...
		for (i=0; i<4; i++)
		{
			...
			tw2968_byte_write(addr, 0x0a + 0x10*i, 0x0a);/* Horizontal Delay Register,	Low  */	/* D1 NTSC:0x01 */
			tw2968_byte_write(addr, 0x0b + 0x10*i, 0xce);/* Horizontal Active Register,	Low  */	/* D1 NTSC:0xC0 */                                   
		}
...
int tw2968_set_audio_samplerate(tw2968_audio_samplerate samplerate)
{
	...
	tw2968_byte_write(TW2968A_I2C_ADDR, 0x40, 0x01);
	...
	tw2968_byte_write(TW2968A_I2C_ADDR, 0x40, 0x00);
...
static void tw2968_set_audio_channel(unsigned int chno)
{
	tw2968_byte_write(tw2968_i2c_addr[0], 0xe0, chno);
}
...
long tw2968_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	{		
		...
		case _IOC_NR(TW2968_GET_VIDEO_LOSS):/*get video loss state */
		{
			...
			{
				//tw2968_byte_write(addr, 0x40, 0x01); //Page index set to 1
				tw2968_byte_write(i2c_addr, 0x40, 0x01); //Page index set to 1
				tmp = tw2968_byte_read(i2c_addr, 0x00+(video_loss.ch-4)*0x10);
				//tw2968_byte_write(addr, 0x40, 0x00); //Page index set to 0
				tw2968_byte_write(i2c_addr, 0x40, 0x00); //Page index set to 0
			}
			else
		...
		case _IOC_NR(TW2968_SET_AUDIO_MIX_OUTSEL):
		{	
			tw2968_audio_mix_outsel audio_mix_outch;	
			if (copy_from_user(&audio_mix_outch, argp, sizeof(tw2968_audio_mix_outsel)))
			  return -1;
			if(audio_mix_outch.chno > 0x7) {
			    printk("invalid audio_chno:%d\n", audio_mix_outch.chno);
			    return -1;	
			}
			tw2968_set_audio_channel(audio_mix_outch.chno);
			break;
		}	
		default:
  		{
...
//	tw2968_byte_write(addr, 0x40, 0x01); //Page index set to 1
	tw2968_byte_write(chip_addr, 0x40, 0x01); //Page index set to 1
	/* 0xD0, 0xD1, 0x7F - Analog Audio Input Gain */
	tw2968_byte_write(chip_addr, 0x7f, 0x88);/* [7:4] AIGAIN5 [3:0] MIXRATIO5 */
	tw2968_byte_write(chip_addr, 0xD0, 0x88);/* [7:4] AIGAIN6 [3:0] AIGAIN5 */
	tw2968_byte_write(chip_addr, 0xD1, 0x88);/* [7:4] AIGAIN8 [3:0] AIGAIN7 */

	//tw2968_byte_write(addr, 0x40, 0x00); //Page index set to 0
	tw2968_byte_write(chip_addr, 0x40, 0x00); //Page index set to 0
	...
	tw2968_byte_write(chip_addr, 0xDB, 0xc1);
	
	/* u-Law/A-Law Output and Mix Mute Control */
	tw2968_byte_write(chip_addr, 0xDC, 0x00);/* PCM output */
	
	/* Mix Output Selection */
	tw2968_byte_write(chip_addr, 0xE0, 0x00);/* Select playback audio of the first stage chips*/
	...
	tw2968_byte_write(chip_addr, 0x70, 0x88);/* [2:0] 0:8k, 1:16k, 2:32k, 3:44.1k 4:48k */ /* Set 0x88 for TW2968 */
	...
	// amplifier  by zzc	
	//tw2968_byte_write(chip_addr, 0xDF, 0xf0); 
	tw2968_byte_write(chip_addr, 0xDF, 0x80); 
}
...
static void tw2968_video_init(unsigned char addr, unsigned char mode)
{
  ...
  //----------------------------------------------------------------------------  
	for (i=0; i<4; i++)
	{
		tw2968_byte_write(addr, 0x10 * i + 0x1, 0x00);	/* brightness.             */
		tw2968_byte_write(addr, 0x10 * i + 0x2, 0x64);	/* luminance contrast gain */
		tw2968_byte_write(addr, 0x10 * i + 0x3, 0x11);	/* sharpness  */             
		tw2968_byte_write(addr, 0x10 * i + 0x4, 0x80);	/* Chroma (U) */             
		tw2968_byte_write(addr, 0x10 * i + 0x5, 0x80);	/* Chroma (V) */             
		tw2968_byte_write(addr, 0x10 * i + 0x6, 0x00);	/* hue        */             
	}    
  ...
  //----------------------------------------------------------------------------  
	...
	/* CLKPO,CLKPN, 4*/
	/* Output Enable Control and Clock Output Control */
	//tw2968_byte_write(addr, 0xfa, 0x4a);/* BT656 Clock freq 27M/36M,54M/72M,or 108M/144M. */ 
//	tw2968_set_work_mode_4d1(addr, 0);    
	tw2968_set_work_mode_4d1(addr, 0);    
	/* CHID*/
//	tw2968_byte_write(addr, 0x9e, 0x62);//    CHID with the specific ITU-R BT.656 sync Code
	tw2968_byte_write(addr, 0x9e, 0x72);//  CHID with the specific ITU-R BT.656 sync & horizontal blanking  code
//	tw2968_byte_write(addr, 0x9e, 0x62);//  CHID with the specific ITU-R BT.656 sync & horizontal blanking  code

	//tw2968_byte_write(addr, 0x9e, 0x42);//72
	tw2968_byte_write(addr, 0x5b, 0x03);//72
	//tw2968_byte_write(addr, 0x9e, 0x7a);//  CHID with the specific ITU-R BT.656 sync & horizontal blanking code
	
    
	/* Video Channel Output Control */
	//tw2968_byte_write(addr, 0xca, 0x02);/* One Channel ITU0R BT.656 */
	//tw2968_byte_write(addr, 0xca, 0xaa);/* Four Channel ITU0R BT.656 Time-multiplexed */ /* Transfer to 0xE7 for TW2968 */
...
//	tw2968_byte_write(addr, 0xeb, 0x54);
//	tw2968_byte_write(addr, 0xee, 0x76);
#endif
	/* Video Miscellaneous Function Control*/
	tw2968_byte_write(addr, 0xf9, 0x00);/* limit output ranges */

	#ifdef TW2968_D1 //D1

	//tw2968_byte_write(addr, 0x6F, 0x03);// Change from 0x50 @ TW2964, BT656 Data mode, 27M or 36M. & Output enable
	tw2968_byte_write(addr, 0x6F, 0x00);// Change from 0x50 @ TW2964, BT656 Data mode, 27M or 36M. & Output enable
	//tw2968_byte_write(addr, 0x62, 0x0);
	tw2968_byte_write(addr, 0xf9, 0x0);	// CLK0P, CLK0N ==> 27/54/108 MHz
	tw2968_byte_write(addr, 0xcc, 0x00);	//Video deocder,H960 enable
	tw2968_byte_write(addr, 0xcd, 0x00);

	#else //WD1

	tw2968_byte_write(addr, 0x6F, 0xf3);	// Change from 0x50 @ TW2964, BT656 Data mode, 27M or 36M. & Output enable
	//tw2968_byte_write(addr, 0x62, 0xf0);
	tw2968_byte_write(addr, 0xf9, 0x03);	// CLK0P, CLK0N ==> 27/54/108 MHz
	tw2968_byte_write(addr, 0xcc, 0xff);	
	tw2968_byte_write(addr, 0xcd, 0xff);

	#endif

	//tw2968_byte_write(addr, 0xfb, 0x6f);//
	tw2968_byte_write(addr, 0xfb, 0x2f);//
	tw2968_byte_write(addr, 0x9f, 0x33);//

	/* For Hi3515 Demo board 2D1 mode */
	// tw2968_byte_write(addr, 0xcc, 0x39);
	// tw2968_byte_write(addr, 0xcd, 0xe4);
...
static void tw2968_tbl_init(unsigned char addr)
{
	unsigned short *parray;
	parray = tbl_tw2968_dump_108m;

	while (parray) {
		unsigned short reg, val;
		if (*parray == ISIL_ENDFLAG) break;
		reg = *parray++;
		val = *parray++;
		tw2968_byte_write(addr, reg, val);
	}
	tw2968_byte_write(addr, 0x40, 0x0);
}
...
static void tw2968_comm_init(unsigned char addr)
{
    ...
    if (0xff == id || 0x00 == id)
    {
        printk("warning: tw2968 0x%x i2c_read err, val:0x%02x!!!\n", addr, id);
        return;
    }
	// add
	if (((id >> 0x3) & 0x1f) != 0x1e) 
		printk("Error device id [0x%02x: 0x%02x]\n", 
			(uint8_t)(id >> 3), (uint8_t)(id & 0x7));
	else 
		printk("Find device tw2968 [0x%02x: 0x%02x]\n", 
			(uint8_t)(id >> 3), (uint8_t)(id & 0x7));
//	id  = tw2968_byte_read(addr, 0xfe);
//	printk("read from reg 0xfe 0x%2x\n", id);
//	tw2968_byte_write(addr, 0xfe, id | 0x3);
//	id  = tw2968_byte_read(addr, 0xfe);
//	printk("read from reg 0xfe 0x%2x\n", id);

	tw2968_tbl_init(addr);
    ...
    //tw2968_byte_write(addr, 0x61, 0x1e);  // TI/4
    tw2968_byte_write(addr, 0x61, 0x1f);  // PLL
	// to set 0x61 bit 4 to 0x1 ??

    /* Enable Video and Audio Detection*/
    tw2968_byte_write(addr, 0xfc, 0xFF);  // ok enable video detection
		//-------------------------------------------------------
    
    tw2968_byte_write(addr, 0x40, 0x01);	// Page index set to 1
		//-------------------------------------------------------
    /* Software Reset*/
    tw2968_byte_write(addr, 0x80, 0x0f);  // reset all
    ...
    /* Analog Clock PLL 144MHz output is used for all  system clock source*/
    // tw2968_byte_write(addr, 0x61, 0x13);
    //tw2968_byte_write(addr, 0x61, 0x1e);
    tw2968_byte_write(addr, 0x61, 0x1f);

    /* Enable Video and Audio Detection*/
    tw2968_byte_write(addr, 0xfc, 0xFF);
		//-------------------------------------------------------

    tw2968_byte_write(addr, 0x40, 0x00);	// Resume Page index to 0
}
...
int tw2968_module_init(void)
{
	...
	ret = init_i2c(i2c_host_id); // select i2c controller 2 ,   
	if (ret) 
	{
		printk("ERROR: could not get i2c adapter 0x%x\n", i2c_host_id);
		return ret;
	}
	...
	// set default mode:
	//           PAL  1d1  scalar(0x0)
	tw2968_set_video_norm(i2c_addr, TW2968_PAL);
#if defined VIDEO_2D1
	tw2968_set_work_mode_2d1(i2c_addr, 0);
#else
#error "Notice, we use 2d1 by default"
	tw2968_set_work_mode_4d1(i2c_addr, 0);
#endif
	//tw2968_set_video_scaler(i2c_addr, 0xc00f30000UL);  // i changed it   
	tw2968_set_video_scaler(i2c_addr, 0xfff0f0f0f0);  // i changed it   
	//tw2968_set_video_scaler(i2c_addr, 0xffeaeaeaea);  // i changed it   
	
	tw2968_set_audio_channel(0x0); // set the audio channel
	...
	tw2968_byte_write(i2c_addr, 0x9f, 0xff);  // ?? 
	printk("tw2968 driver init successful!\n");
	
	return 0;
}

//void __exit tw2968_module_exit(void)
void tw2968_module_exit(void)
{
...
MODULE_DESCRIPTION("TW2968 video driver");
MODULE_LICENSE("GPL");
*/
